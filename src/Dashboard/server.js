module.exports.start = (client) => {
  ...
  const encryptor = require("simple-encryptor")(
    "asdasklvmopqhqdviöojqisdmladm?_**12156454"
  );
  ...
  app.get("/callback", async (req, res) => {
    ...
    const user = await oauth.getUser(data.access_token).catch((err) => {
      res.render("errorPages/404");
    });
  ...
  app.listen(process.env.PORT, () => {
    console.log(`Dashboard started at port ${process.env.PORT}.`.blue);
  });
};
